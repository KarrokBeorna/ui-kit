...
export function Pagination({
  ...
  const goTo = useCallback(
    (p: number) => {
      ...
      if (externalPage === undefined) setInternalPage(newPage);
  ...
  if (isMobile) {
    return (
      <MobilePagination
        ...
        total={total}
        onPageChange={(p, pp) => {
          if (externalPage === undefined) setInternalPage(p);
          setPerPage(pp);
          onPageChange?.(p, pp);
        }}
      />
  ...
  // ── Десктопный режим (без изменений) ──────────────
  const pages: (number | '...')[] = [];
  if (totalPages <= 7) {
    ...
    >
        ...
        <div style={{ position: 'relative' }} ref={dropRef}>
          <button
            ...
            style={{
              ...
              gap: 6,
              color: t.text,
              background: dropOpen ? t.navHoverBg : t.bgSurface,
              borderColor: dropOpen ? t.accent : t.border,
            ...
            {[10, 50, 100, 500, 1000].map((n) => (
              <button
                ...
                style={{
                  ...
                  padding: '7px 12px',
                  textAlign: 'left',
                  background: perPage === n ? t.navHoverBg : 'transparent',
                  border: 'none',
                  borderRadius: 7,
                  fontSize: 13,
                  fontFamily: 'system-ui',
                  color: perPage === n ? t.accent : t.text,
                  cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <button
          ...
          onClick={() => goTo(page - 1)}
          disabled={page === 1}
          style={{ ...btn, opacity: page === 1 ? 0.35 : 1, cursor: page === 1 ? 'not-allowed' : 'pointer' }}
        >
          <IcoChevronLeft s={14} />
        </button>
        {pages.map((p, i) =>
          p === '...' ? (
            <span key={`e${i}`} style={{ color: t.textMuted, fontSize: 13, padding: '0 4px', userSelect: 'none' }}>
              …
            </span>
          ) : (
            <button
              ...
              style={{
                ...btn,
                background: page === p ? t.accent : t.bgSurface,
                color: page === p ? t.accentText : t.textMuted,
                borderColor: page === p ? 'transparent' : t.border,
                fontWeight: page === p ? 700 : 500,
                boxShadow: page === p ? `0 2px 12px ${t.accentGlow}` : 'none',
              }}
            >
              {p}
            </button>
        ...
        <button
          ...
          style={{ ...btn, opacity: page === totalPages ? 0.35 : 1, cursor: page === totalPages ? 'not-allowed' : 'pointer' }}
        >
